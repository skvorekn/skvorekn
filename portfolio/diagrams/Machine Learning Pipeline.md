```mermaid
flowchart TD
    raw_train_data@{ shape: docs, label: "Raw training data" }-->transform([Transformation code suite])-- data quality and feature engineering iterations with stakeholders -->raw_train_data
    transform-- model training -->model((Trained model))-- tuning based on model evaluation and bias assessment -->transform
    monitoring([Model monitoring])-- re-training -->model
    linkStyle 0,1,2,3 stroke:#A89234,color;

    raw_test_data@{ shape: docs, label: "Raw prediction data" }-- validation -->transform
